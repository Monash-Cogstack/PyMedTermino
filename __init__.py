...
class ModifiedConcept(Concept):
  def __init__(self, origin, mods, term = None):
    term = term or origin.term
    self.code        = "%s:%s" % (origin.code, u":".join(modifier.code for modifier in mods))
    self.origin      = origin
    self.mods        = mods
  ...
  def __rshift__(self, destination_terminology):
    ...
    if destination_terminology.name == u"VCM":
      ...
      #vcm_modifiers = [modifier for modifier in self.mods if modifier.terminology.name == u"VCM"]
      vcm_lexs      = [lex for modifier in self.mods if modifier.terminology.name == u"VCM" for lex in modifier.lexs if not lex.empty]
      return Concepts(mapped.derive_lexs(vcm_lexs) for mapped in mappeds)
  
    return Concepts(ModifiedConcept(mapped, self.mods) for mapped in (self.terminology >> destination_terminology)(self.origin))
  
  def __hash__(self): return hash((self.origin, frozenset(self.mods), self.term))
  
  def __eq__(self, other):
    if not isinstance(other, ModifiedConcept): return False
    return (self.origin == other.origin) and (self.mods == other.mods)
  
  def is_a(self, other):
    if not isinstance(other, ModifiedConcept): return None
    if not self.origin.is_a(other.origin): return None
    for modificateur_o in other.mods:
      for modificateur_s in self.mods:
        if modificateur_s.is_a(modificateur_o): break
      else: return None
    return 1
  imply = is_a
  ...
  def full_code(self):
    if self.term != self.origin.term: return u":".join([self.origin.full_code()] + [modifier.full_code() for modifier in self.mods] + [self.term])
    return u":".join([self.origin.full_code()] + [modifier.full_code() for modifier in self.mods])
